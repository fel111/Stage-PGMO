...
void lotSizComp(int cardT, int qmax, vector<int> dt, vector<vector<int> > bpt, vector<vector<float> > valbpt, vector<vector <float> > pente,vector<int> nb_bp){
	SCIP * scip;
	SCIPcreate(&scip);
	...
	for(int i=0; i<cardT; ++i){
		SCIP_VAR * var;
		st.push_back(var);
		SCIPcreateVarBasic(scip, &st[i], ("st"+to_string(i)).c_str(), 0, qmax, 0.0, SCIP_VARTYPE_INTEGER);
	...
	for(int i=0; i<cardT; ++i){
		...
		for(int j=0; j<nb_bp[i]; ++j){
			...
			SCIPaddCoefLinear(scip, cons_ct[i], xtemp[i][j],-pente[i][j]);
	...
	for(int i=0; i<cardT; ++i){
		...
		for(int j=0; j<nb_bp[i]; ++j){
			...
			SCIPaddCoefLinear(scip, cons_z2[i][j], ztj[i][j],-bpt[i][j+1]+1);
	...
	// contrainte xtemp_ij = x_ij - bpt_ij
	...
	for(int i=0; i<cardT; ++i){
		...
		for(int j=0; j<nb_bp[i]; ++j){
			...
			SCIPaddCoefLinear(scip, cons_xtemp[i][j], ztj[i][j],bpt[i][j]);
