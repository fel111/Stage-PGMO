...
SCIP_RETCODE Load_Original_Model (structGenCol & sGC)
{
	...
	for(int l=0; l<sGC.cardL; ++l){
		...
		for(int k=0; k<sGC.d.nb_bp[0]; ++k){
			...
			for(int t=0; t<sGC.d.cardT; ++t){
				...
				SCIPcreateVarBasic(scip, &y_lkt[l][k][t], ("y_lkt"+to_string(l)+to_string(k)+to_string(t)).c_str(), 0.0, SCIP_REAL_MAX, SCIP_REAL_MAX, SCIP_VARTYPE_CONTINUOUS);
				SCIPaddVar(scip,y_lkt[l][k][t]);		
			}
		}	
	}
	sGC.varY_lkt = y_lkt;
	...
	for(int k=0; k<sGC.d.nb_bp[0]; ++k){
		...
		for(int t=0; t<sGC.d.cardT; ++t){
			...
			SCIPcreateVarBasic(sGC.scip, &(y0_kt[k][t]), ("y0_kt"+to_string(k)+to_string(t)).c_str(), 0, SCIP_REAL_MAX, SCIP_REAL_MAX, SCIP_VARTYPE_CONTINUOUS);
	...
	for(int i=0; i<sGC.d.cardJ; ++i){
		SCIP_CONS * c;
		cons_2.push_back(c);
		SCIPcreateConsLinear(sGC.scip, &cons_2[i], "cons_2", 0, 0, 0, sGC.d.pj[i], SCIPinfinity, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, TRUE, FALSE, FALSE, FALSE);  
		for(int t=0; t<sGC.d.cardT; ++t){
			for(const auto& l : sGC.L_t[t]){
				for(const auto& k : sGC.K_l[l]){
	...
	for(int t=0; t<sGC.d.cardT; ++t){
		SCIP_CONS * c;
		cons_5.push_back(c);
		SCIPcreateConsLinear(sGC.scip, &cons_5[t], "cons_5", 0, 0, 0, -SCIPinfinity, 1, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE);  
		SCIPaddCoefLinear(sGC.scip, cons_5[t], z0_pt[p][t],1);
		for(int p=0; p<(sGC.d.nb_bp[0]/2); ++p){
			SCIPaddCoefLinear(sGC.scip, cons_5[t], y0_kt[p*2][t],-1);
			SCIPaddCoefLinear(sGC.scip, cons_5[t], y0_kt[p*2+1][t],-1);
		}
		SCIPaddCons(sGC.scip, cons_2[i]);
	}
	...
	// q_max - q0 >= 0
	SCIP_CONS * cons_7;
	SCIPcreateConsLinear(sGC.scip, &cons_7, "cons_7", 0, 0, 0, 0, SCIPinfinity, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE);  
	...
	for(int t=0; t<sGC.d.cardT; ++t){
		...
		for(const auto& l : sGC.L_t[t]){
			for(const auto& k : sGC.K_l[l]){
				SCIPaddCoefLinear(sGC.scip, cons_8[t], y_lkt[l][k][t],(sGC.L[l].energyDemand-sGC.d.bpt[k]));
				SCIPaddCoefLinear(sGC.scip, cons_8[t], y0_kt[k][t],-sGC.d.bpt[k]);
			}
		}
